...
const Loginform = () => {
  ...
  try {
    ...
    if (response?.token) {
      ...
      console.error("Error: ", response?.data?.message || "Invalid response format");
    ...
    <div>
      <div className="form-wrapper">
        <div className="w-full">
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} className="bg-white rounded px-8  main-form">
      ...
      >
        Login
      </button>            <span className="text-for-logn">
          ...
          <button onClick={
            ...
            <Image src={googleicon} alt="Logo" className="google-icon-image icon" />
            </button>
        }</div>
          <div className="bottom-text-wrapper">
            <span className="text-for-logn">Dont Have an accounts?</span>{" "}
          
             <button  className="click-sign-upp" onClick={() => router.push('/app-components/signup')}>Click here to sign up</button>
          </div>
